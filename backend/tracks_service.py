...
try:
    ...
    print('*' * 20)
    print('NO TOKENS FOUND: Visit /api/request_token to get tokens')
    print('*' * 20)
...
def initialize():
    if access_token == None or refresh_token == None:
        ...
        code = requests.get('https://accounts.spotify.com/authorize', params={'client_id':client_id, 'response_type':'code', 'redirect_uri':f'{BASE_URL}/api/request_token/callback', 'scope':'user-read-recently-played'})
...
def home():
    ...
    code = requests.get('https://accounts.spotify.com/authorize', 
                params={'client_id':client_id, 
                        'response_type':'code', 
                        'redirect_uri':f'{BASE_URL}/api/request_token/callback', 
                        'scope':'user-read-recently-played'})
    print('redirecting to ' + code.url)
...
def callback():
    ...
    r = requests.post('https://accounts.spotify.com/api/token', data={'grant_type':'authorization_code', 'code':code, 'redirect_uri':f'{BASE_URL}/api/request_token/callback', 'client_id':client_id, 'client_secret':client_secret})
    print(r.json())
...
@app.route('/api/get_daily_mean_by_days')
def get_daily_mean_by_days():
    ...
    try:
        ...
        FROM tracks
        WHERE play_date >= date('now','-{days} days')
